...
import { SpecialtyTypesService } from './specialty-types.service';
import { CreateSpecialtyTypeDto, UpdateSpecialtyTypeDto } from './dto';
...
export class SpecialtyTypesController {
  constructor(@Inject(SpecialtyTypesService) private service: SpecialtyTypesService) {}
  ...
  create(
    @Body() dto: CreateSpecialtyTypeDto,
  ...
  update(
    @Param('id') id: string,
    @Body() dto: UpdateSpecialtyTypeDto,
    @CurrentUser() user: RequestUser,
    @CurrentTenant() tenantId: string,
  ) {
    const scope = user.globalRole === 'SUPER_ADMIN' && !tenantId ? null : tenantId;
