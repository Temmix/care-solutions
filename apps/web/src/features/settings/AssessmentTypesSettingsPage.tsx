...
export function AssessmentTypesSettingsPage(): React.ReactElement {
  const { isSuperAdmin } = useAuth();
  const [types, setTypes] = useState<AssessmentTypeConfig[]>([]);
  ...
  const fetchTypes = useCallback(async () => {
    setLoading(true);
    try {
      const endpoint = isSuperAdmin ? '/assessment-types/system' : '/assessment-types/tenant';
      const result = await api.get<AssessmentTypeConfig[]>(endpoint);
      setTypes(result);
    } catch {
  ...
  const handleEdit = (t: AssessmentTypeConfig) => {
    setEditingId(t.id);
    setForm({
      code: t.code,
      name: t.name,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await api.patch(`/assessment-types/${editingId}`, {
          name: form.name.trim(),
  ...
  const handleDeactivate = async (id: string) => {
    if (!confirm('Are you sure you want to deactivate this assessment type?')) return;
    try {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Assessment Types</h1>
          <p className="text-sm text-slate-500 mt-1">
            {isSuperAdmin
              ? 'Manage system-wide assessment types'
              : 'Manage custom assessment types for your organisation'}
          </p>
        </div>
        <button
      ...
      {showForm && (
        <div className="mb-6 bg-white rounded-xl border border-slate-100 p-6">
          <h2 className="text-sm font-semibold text-slate-900 mb-4">
            {editingId ? 'Edit Assessment Type' : 'New Assessment Type'}
          </h2>
