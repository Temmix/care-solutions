...
export function SpecialtyTypesSettingsPage(): React.ReactElement {
  const { isSuperAdmin } = useAuth();
  const [types, setTypes] = useState<SpecialtyTypeConfig[]>([]);
  ...
  const fetchTypes = useCallback(async () => {
    setLoading(true);
    try {
      const endpoint = isSuperAdmin ? '/specialty-types/system' : '/specialty-types/tenant';
      const result = await api.get<SpecialtyTypeConfig[]>(endpoint);
      setTypes(result);
    } catch {
      setError('Failed to load specialty types');
    } finally {
      setLoading(false);
    }
  }, [isSuperAdmin]);
  ...
  const handleEdit = (t: SpecialtyTypeConfig) => {
    setEditingId(t.id);
    setForm({
      code: t.code,
      name: t.name,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await api.patch(`/specialty-types/${editingId}`, {
          name: form.name.trim(),
        ...
        await api.post('/specialty-types', {
  ...
  const handleDeactivate = async (id: string) => {
    if (!confirm('Are you sure you want to deactivate this specialty type?')) return;
    try {
      await api.delete(`/specialty-types/${id}`);
      fetchTypes();
    } catch (err) {
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Specialty Types</h1>
          <p className="text-sm text-slate-500 mt-1">
            {isSuperAdmin
              ? 'Manage system-wide practitioner specialty types'
              : 'Manage custom specialty types for your organisation'}
          </p>
        </div>
        <button
      ...
      {showForm && (
        <div className="mb-6 bg-white rounded-xl border border-slate-100 p-6">
          <h2 className="text-sm font-semibold text-slate-900 mb-4">
            {editingId ? 'Edit Specialty Type' : 'New Specialty Type'}
          </h2>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. GENERAL_PRACTICE"
                  disabled={!!editingId}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Medical, Nursing, Allied Health"
                  className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm"
                />
              </div>
              <div>
