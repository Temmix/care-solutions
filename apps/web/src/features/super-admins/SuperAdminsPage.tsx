...
function validateForm(form: CreateSuperAdminForm): FieldErrors {
...
const emptyForm: CreateSuperAdminForm = {
  email: '',
  password: '',
  firstName: '',
  lastName: '',
...
interface CreateModalProps {
  ...
  saving: boolean;
  onClose: () => void;
  onSave: (form: CreateSuperAdminForm) => void;
}
...
function CreateSuperAdminModal({
  ...
  const [form, setForm] = useState<CreateSuperAdminForm>(emptyForm);
  const [touched, setTouched] = useState<Record<string, boolean>>({});

  const errors = useMemo(() => validateForm(form), [form]);
  const isValid = Object.keys(errors).length === 0;

  const set = (field: keyof CreateSuperAdminForm) => (e: React.ChangeEvent<HTMLInputElement>) =>
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
        <h2 className="text-lg font-semibold text-slate-900 mb-4">Add Super Admin</h2>
...
export function SuperAdminsPage(): React.ReactElement {
  const { user } = useAuth();
  const { list, create, deactivate, reactivate, loading, error } = useSuperAdmins();
  ...
  const handleCreate = async (form: CreateSuperAdminForm) => {
    setSaving(true);
    setActionError('');
    try {
      await create(form);
      ...
      setActionError(err instanceof Error ? err.message : 'Failed to create super admin');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div>
      ...
      {loading && admins.length === 0 && (
        <div className="flex items-center justify-center py-20">
          <div className="text-slate-400 text-sm">Loading super admins...</div>
        </div>
      )}
      ...
      {!loading && admins.length === 0 && (
        <div className="bg-white rounded-xl border border-slate-100 text-center py-16">
          <div className="text-slate-400 text-sm">No super admins found</div>
        </div>
      )}
