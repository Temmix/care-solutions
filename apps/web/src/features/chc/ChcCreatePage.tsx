...
import { useChc } from './hooks/use-chc';
...
const FAST_TRACK_REASONS = ['TERMINAL_ILLNESS', 'RAPIDLY_DETERIORATING'] as const;

function validate(f: FormData): Record<string, string> {
  const errors: Record<string, string> = {};
  if (!f.patientId) errors.patientId = 'Patient is required';
  if (!f.referralReason.trim()) errors.referralReason = 'Referral reason is required';
  if (f.isFastTrack && !f.fastTrackReason) {
    errors.fastTrackReason = 'Fast track reason is required when fast-tracking';
  }
  return errors;
}

export function ChcCreatePage(): React.ReactElement {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const { createCase } = useChc();
  ...
  async function handleSubmit(e: React.FormEvent): Promise<void> {
    ...
    try {
      await createCase({
        ...
        isFastTrack: form.isFastTrack,
        fastTrackReason: form.isFastTrack ? form.fastTrackReason : undefined,
      });
      navigate('/app/chc');
    } catch (err) {
      setSaveError(err instanceof Error ? err.message : 'Failed to create referral');
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold text-slate-900 mb-6">New CHC Referral</h1>
      ...
      >
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">
            Encounter <span className="text-slate-400">(optional)</span>
          </label>
          <select
          ...
          <p className="text-xs text-slate-400 mt-1">
            Link this referral to an existing patient encounter
          </p>
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            onBlur={() => blur('referralReason')}
            className={inputCls('referralReason')}
            rows={3}
            placeholder="Describe the reason for CHC referral"
          />
          {touched.referralReason && fieldErrors.referralReason && (
            <p className="text-red-500 text-xs mt-1">{fieldErrors.referralReason}</p>
          )}
        </div>
        ...
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              type="checkbox"
              checked={form.isFastTrack}
              onChange={(e) => set('isFastTrack', e.target.checked)}
              className="rounded border-slate-300"
            />
            <span className="text-sm font-medium text-slate-700">Fast Track</span>
          </label>
        </div>
        ...
        {form.isFastTrack && (
          <div>
            ...
            <select
              value={form.fastTrackReason}
              onChange={(e) => set('fastTrackReason', e.target.value)}
              onBlur={() => blur('fastTrackReason')}
              className={inputCls('fastTrackReason')}
            >
              <option value="">Select reason</option>
              {FAST_TRACK_REASONS.map((r) => (
                <option key={r} value={r}>
                  {r.replace(/_/g, ' ')}
                </option>
              ))}
            </select>
            {touched.fastTrackReason && fieldErrors.fastTrackReason && (
              <p className="text-red-500 text-xs mt-1">{fieldErrors.fastTrackReason}</p>
            )}
          </div>
        )}
